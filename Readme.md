...
- **Knowledge Base:** The RAG's knowledge is sourced from markdown files located in the `simpleRag/data/` directory. Documents are automatically indexed with inferred intents (e.g., budgeting, investing) for better retrieval context.

- **API Endpoints:** The service runs on `http://localhost:8000` and exposes a REST API:

  - `POST /chat`: The main endpoint for sending user questions. It accepts a `question` and a `session_id`.
  - `POST /rag/retrieve`: (New) Direct endpoint for retrieving document contexts, primarily used for evaluation.
...
- **Quality Evaluation:** The system includes a comprehensive evaluation suite using **Ragas**, measuring Faithfulness, Answer Relevancy, Context Recall, and Context Precision.

- **More Information:** For detailed setup, configuration, and evaluation instructions, please refer to the [simpleRag/README.md](simpleRag/README.md).
